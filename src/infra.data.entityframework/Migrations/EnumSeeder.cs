...
namespace KlinderRH.Social.Infra.Data.EntityFramework.Migrations
{
	public static class EnumSeeder
	{
			...
			string tableName = null)
		{
			if (tableName == null)
			{
				tableName = typeof (TEnum).Name + "Enum";
			}
		...
		private static IDictionary<string, string> GetDescriptions<TEnum>()
		{
			return typeof (TEnum).GetMembers(BindingFlags.Static | BindingFlags.Public)
				.Select(m => new
				{
					m.Name,
					Description = m.Name
					// m.GetCustomAttributes(typeof (DescriptionAttribute), true).Cast<DescriptionAttribute>().SingleOrDefault()
				})
				.ToDictionary(a => a.Name, a => a.Description); // ?.Description
		}
